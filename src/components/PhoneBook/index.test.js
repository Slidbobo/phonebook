...
import {
    findByRole,
    fireEvent, getAllByLabelText, getAllByRole,
    getByLabelText,
    getByRole,
    getByText,
    render,
...
const addContactFn = async () => {
    ...
    const inputFirstName = getByRole(contactAddModal, 'textbox', { name: 'First Name' })
    fireEvent.change(inputFirstName, { target: { value: "John"}})

    // Searches and fills the text field for the last name in the modal
    const inputLastName = getByRole(contactAddModal, 'textbox', { name: 'Last Name' })
    fireEvent.change(inputLastName, { target: { value: "Doe"}})

    // Opens the options of the label for the first phone number in the modal
    const firstLabel = getByRole(getByLabelText(contactAddModal, 'phoneNumbers-0-label'), 'button')
    fireEvent.mouseDown(firstLabel)

    // Searches, fills and verifies the text fields for the phone numbers in the modal
    getAllByRole(contactAddModal, 'textbox', { name: "Phone Number" }).map((phoneNumber, index) => {
        fireEvent.change(phoneNumber, { target: { value: '+49 (0)171 / 987 654 321'}})
    })
    ...
    await waitForElementToBeRemoved(() => screen.queryByLabelText("modal-add-contact"))
}
...
test('Open modal and add a contact with two phone numbers', async () => {
    ...
    expect(getAllByLabelText(contactAddModal, /phoneNumbers-[0-9]+-label/i)).toHaveLength(1)
    ...
    const allPhoneNumberLabels = getAllByLabelText(contactAddModal, /phoneNumbers-[0-9]+-label/i)
    ...
    fireEvent.click(allOptions.reduce((prevOption, currentOption) => {
        if (currentOption.dataset.value === "work")
    ...
    // Searches, fills and verifies the text fields for the phone numbers in the modal
    getAllByRole(contactAddModal, 'textbox', { name: "Phone Number" }).map((phoneNumber, index) => {
        const inputValue = index ? '+49 (0)30 / 123 456 789' : '+49 (0)171 / 987 654 321'
    ...
    expect(getByText(contactTableBody, '+49 (0)30 / 123 456 789')).toBeInTheDocument()
    expect(getByText(contactTableBody, 'Mobile')).toBeInTheDocument()
    expect(getByText(contactTableBody, '+49 (0)171 / 987 654 321')).toBeInTheDocument()
})